...
SR_COLS = {
    ...
    "sr_conf": st.column_config.TextColumn(
        ...
        help="**The price level BOTH your charts agree on \u2014 with the stock sitting on it "
             ...
             "charts agree on goes **\u22120.89% over a month** and was negative in **8 years "
             "out of 8**, while a randomly placed fake level did nothing at all. A shelf "
             "everybody can see is a shelf that **BREAKS**.\\n\\n"
             "\u23f3 **A SLOW effect \u2014 not a next-day one.** The day-after figure "
             "(\u22120.21%) does NOT survive once you allow for hundreds of stocks sharing each "
             "date and moving together: tested date by date, only 53% of days come out negative "
             "(p=0.53). The monthly result does survive (62% of days, p=0.014). Use this to "
             "place the stop and size the trade, never to time tomorrow.\\n\\n"
             "\u23f1\ufe0f **And the list churns** \u2014 73% of the time a name qualifies for "
             "exactly ONE session (measured on the slowest pair), so re-scan rather than "
             "carrying it forward."),
    ...
    if view == "🔬 Structure scan":
        ...
        if _P:
            ...
            _conf_help = ("**Show me only stocks that are SITTING ON a price level BOTH of my "
                      ...
                      "| if you buy it | agreed level | fake level | holds up? |\n"
                      "|---|---|---|---|\n"
                      "| next day | −0.21% | +0.05% | **no** |\n"
                      "| after a week | −0.29% | +0.12% | marginal |\n"
                      "| **after a month** | **−0.89%** | −0.16% | **yes** |\n"
                      "| overnight only | **0.00%** | −0.03% | flat either way |\n\n"
                      "**The `holds up?` column is not decoration.** The raw numbers "
                      "treat every stock-day as an independent draw, and they are not "
                      "\u2014 hundreds of names share each date and move with the market "
                      "together. Re-tested date by date, the **next-day** number falls "
                      "apart (only 53% of dates negative, p=0.53) while the **monthly** "
                      "one survives (62% of dates, p=0.014, and negative in 8 years out "
                      "of 8). So this is a **slow** effect: an aligned support "
                      "underperforms over WEEKS. Do not read it as a next-day signal.\n\n"
                      "Read the columns together. **The fake level does nothing** — "
                      "which is how you know the real one is not a fluke — and the real "
                      "one lost in **8 years out of 8** at the monthly horizon.\n\n"
                      "⏱️ **THE LIST CHURNS DAILY.** Measured on the slowest pair "
                      "(1D+1W): **73% of the times a name qualifies it qualifies for "
                      "exactly ONE session**, and only 1% stay five sessions or more. "
                      "Even within a flagged day the median name spent only ~68% of its "
                      "range inside the zone. Treat this as a fresh scan every session, "
                      "never a watchlist that persists — and expect the faster horizons "
                      "to churn harder still.\n\n"
